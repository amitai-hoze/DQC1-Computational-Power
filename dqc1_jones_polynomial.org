...
upper i = metafont $ p2 (i,0).- leaving unitY <> arriving unitY -.endpt (p2 (i+1,2))

lower i = (metafont $ p2 (i+1,0).- leaving unitY <> arriving (r2 (-0.28-0.55,2-0.9)) -.endpt (p2 (i+0.55,0.9))) <>
          (metafont $ p2 (i+0.45,1.1).- leaving (r2 (-0.28-0.45,2-1.1)) <> arriving unitY -.endpt (p2 (i,2)))


fromBraidGen (S i) = upper % i <> lower % i <> (mconcat [line % j | j <- [1..(i-1)] ++ [(i+2)..n]])
